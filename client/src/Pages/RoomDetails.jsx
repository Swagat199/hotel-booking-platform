...
function RoomDetails() {
    ...
    const checkAvailbility = async ()=>{
        try {
            ...
            const {data} = await axios.post('/api/bookings/check-availbility',
            {room:id,checkInDate,checkOutDate})

            if(data.success){
                if(data.isAvailble){
                    setIsAvailble(true);
                    toast.success('Room is availble');
                }else{
                    setIsAvailble(false);
                    toast.error('Room is not availble');
                }
            }else{
        ...
        <div className='flex flex-col md:flex-row md:justify-between mt-10'>
            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between
             bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
                ...
                transition-all text-white ronded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 
                text-base cursor-pointer'>
                    {isAvailble ?"Book Now":"Check Availbility"}
                </button>
            </form>
            ...
            <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
                <p>
                    Guests wiil be allocated on the ground floor accronding to availbility.
                    You get a comfortable Two bedroom apartment has a true city feeling .The
                    pricee quoted is for two guest,at the guest slot please mark the number of Guests
                    to get the exact price for groups.The Guests will be allocated ground floor accroding to 
                    availbility. You get the comfortable two bedroom apartment that has a true city feeling. 
                </p>
            </div>
