...
const Navbar = () => {
    ...
    return (
            <nav 
            className={`fixed top-0 left-0  w-full flex items-center justify-between px-4 md:px-16 
            ...
            "py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    {navLinks.map((link, i) => (
                        <a key={i} href={link.path} className={`group flex flex-col gap-0.5 
                            ...
                            {link.name}
                            <div className={`${isScrolled ? "bg-gray-700" : "bg-white"} h-0.5 
                ...
                <div className="hidden md:flex items-center gap-4">
                   ...
                   className={`${isScrolled && 'invert'} h-7 transition-all duration-500}`}/>
                    ...
                    (<UserButton>
                        ...
                        onClick={openSignIn} 
                        className={`px-8 py-2.5 rounded-full ml-4 transition-all 
                        duration-500 cursor-pointer ${isScrolled ? "text-white bg-black" :
                        "bg-white text-black"}`}>
                        Login
                    </button>)}
                ...
                duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    ...
                    { user && <button 
                    onClick={()=> isOwner ? navigate('/owner')
